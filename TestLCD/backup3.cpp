...
//        //f[i][0] i天 买了股票的价格 f[i][0] = min(f[i - 1][0], prices[i]);
//        //f[i][1] i天 卖了股票的利润 f[i][1] = max(f[i - 1][1], prices[1] - f[i-1][0]);
//
//        vector<vector<int>> result(prices.size(), vector<int>(2, INT_MAX));
//        result[0][0] = prices[0];
//        result[0][1] = 0;
//
//        for (int i = 1; i < prices.size(); i++) {
...
//
//    int maxProfit2(vector<int>& prices) {
//        //f[i][0] i天持有股票时的现金 f[i][0] = max(f[i - 1][0], f[i - 1][1] - p[i]);
//        //f[i][1] i天未持有股票时的现金 f[i][1] = max(f[i - 1][1], p[i] + f[i - 1][0]);
//
//        vector<vector<int>> result(prices.size(), vector<int>(2, 0));
//        result[0][0] = -prices[0];
//        result[0][1] = 0;
...
//            result[1] = max(result[1], temp + prices[i]);
//        }
//
//        return result[1];
//    }
...
//                if (prices[i] > prices[i + 1]) {//开始下跌
//
//                    int last0Num = prices[i] - prices[result[0][0]];
//                    updateValue(result, last0Num, minNum, true);
//
//                    int last1Num = prices[i] - prices[result[1][1]];
//                    updateValue(result, last1Num, minNum, true);
//
//                    int currNum = prices[i] - prices[minNum];
//                    updateValue(result, currNum, minNum, false);
//
//                    minNum = -1;
//                }
//            }
...
//    }
//
//    void updateValue(vector<vector<int>>& result, int val, int index, bool isOldIndex) {
//        if (val > result[0][1]) {
//            result[1] = result[0];
//            result[0][0] = index;
//            result[0][1] = val;
//        }
//        else if (val > result[1][1]) {
//            result[1][0] = index;
//            result[1][1] = val;
//        }
//    }
//
//    int maxProfit2(vector<int>& prices) {
//        //0没有操作，1第一次买入，2第一次卖出，3第二次买入，4第二次卖出
//        /**
//         f[i][0] = f[i - 1][0];
//         f[i][1] = -p[i];
//         f[i][2] = p[i] + f[i - 1][1];
...
//         f[i][4] = p[i] + f[i - 1][3];
//         */
//        vector<vector<int>> result(prices.size(), vector<int>(5, 0));
//
//        result[0][1] = -prices[0];
//        result[0][3] = -prices[0];
//
...
//            result[i][2] = max(result[i - 1][2], prices[i] + result[i - 1][1]);
//            result[i][3] = max(result[i - 1][3], result[i - 1][2] - prices[i]);
//            result[i][4] = max(result[i - 1][4], prices[i] + result[i - 1][3]);
//        }
//
//        return result[prices.size() - 1][4];
//    }
//
//    int maxProfit3(vector<int>& prices) {
//
...
//
//        for (int i = 1; i < prices.size(); i++) {
//            result[3] = max(result[3], result[2] + prices[i]);
//            result[2] = max(result[2], result[1] - prices[i]);
//            result[1] = max(result[1], result[0] + prices[i]);
...
//
//    void test() {
//        vector<int> arr = {1,2,4,2,5,7,2,4,9,0};
//
//        int result = maxProfit3(arr);
//
//        cout<<result<<endl;
...
//public:
//    int maxProfit(vector<int>& prices) {
//        //f[i][0] i天买入的现金 f[i - 2][1] - p[i]
//        //f[i][1] i天卖出的现金 p[i] + f[i - 1][0];
//
//        if (prices.size() <= 1) {
//            return 0;
//        }
...
//        result[0][1] = 0;
//
//        result[1][0] = max(result[0][0], -prices[1]);
//        result[1][1] = max(result[0][1], result[0][0] + prices[1]);
//
//        for (int i = 2; i < prices.size(); i++) {
//            result[i][0] = max(result[i - 1][0], result[i - 2][1] - prices[i]);
//            result[i][1] = max(result[i - 1][1], result[i - 1][0] + prices[i]);
//        }
//
//        return result[prices.size() - 1][1];
...
//
//        for (int i = 2; i < prices.size(); i++) {
//            vector<int> temp = result[1];
//
//            result[1][1] = max(result[1][1], result[1][0] + prices[i]);
//            result[1][0] = max(result[1][0], result[0][1] - prices[i]);
//
//            result[0] = temp;
//        }
//
...
//public:
//    int maxProfit(vector<int>& prices, int fee) {
//        //f[i][0] i天买入后的现金 f[i][0] = max(f[i - 1][0], f[i - 1][1] - p[i])
//        //f[i][1] i天卖出后的现金 f[i][1] = max(f[i - 1][1], p[i] + f[i - 1][0] - fee);
//
//        vector<int> result(2, 0);
//
//        result[0] = -prices[0];
...
//            int temp = result[0];
//            result[0] = max(result[0], result[1] -prices[i]);
//            result[1] = max(result[1], temp + prices[i] - fee);
//        }
//
//        return result[1];
//    }
...
//        //f[n] = max(f(n - 1), f(n - 2),.....) if nums[i] > nums[i - 1] + 1
//
...
//        //f[n] n位置之前连续子序列的长度，包含n，f[n] = if nums[n] > nums[n - 1]: f[n - 1] + 1 else 1;
//
//        if (nums.size() <= 1) {
//            return (int)nums.size();
//        }
...
//
//        int result = findLengthOfLCIS2(arr);
//
//        cout<<result<<endl;
//    }
//};
...
//        //f(i,j)//nums1前i个数和nums2前j个数中公共子序列的长度，包含i,j
//        //f[i, j] = if nums1[i] == nums2[j] : f[i - 1][j - 1] + 1 else 0;
//
//        vector<vector<int>> result(nums1.size(), vector<int>(nums2.size(), 0));
//
//        int maxLength = 0;
//
...
//                    }
//                    maxLength = max(maxLength, result[j]);
//                }
//                else {
//                    result[j] = 0;
//                }
...
//
//        int result = findLength2(nums1, nums2);
//
//        cout<<result<<endl;
//    }
//};
...
//        //f[i][j] text1前i个字符中和text2前j个字符中 子序列长度
//        //f[i][j] = if (text1[i] == text2[j]) f[i - 1][j - 1] + 1 else max(f[i - 1][j], f[i][j - 1]);
//
//        vector<vector<int>> result(text1.size(), vector<int>(text2.size(), 0));
...
//
//        tool.printMatrix(result);
//        return result[text1.size() - 1][text2.size() - 1];
//    }
//
//    int longestCommonSubsequence2(string text1, string text2) {
//        //优化
//
//        vector<int> result(text2.size());
//
//        for (int i = 0; i < text1.size(); i++) {
//            int last = 0;
//            for (int j = 0; j < text2.size() ; j++) {
...
//
//        int result = longestCommonSubsequence2(text1, text2);
//
//        cout<<result<<endl;
//    }
//};
...
//        //最长子序列
//
//        //f(i,j) A中前i个数与B中前j个数的最长公共子序列长度
//        //f[i][j] = if A[i] == B[j] : f[i - 1][j - 1] + 1 else max(f[i - 1][j], f[i][j - 1]);
//
...
//                }
//                else {
//                    int lastI = max(i - 1, 0), lastJ = max(j - 1, 0);
//                    result[i][j] = max(result[lastI][j], result[i][lastJ]);
//                }
//            }
//        }
//
...
//        //f[n] 0-n前子序列最大和，f[n] = max(f[n - 1] + nums[n], nums[n]);
//
//        vector<int> result(nums.size(), 0);
//        result[0] = nums[0];
//        int maxSum = INT_MIN;
...
//        //f(i,j)//s中i 是否是 t中j的子序 = f(i, j - 1) || (s[i] == t[j] && f(i - 1, j - 1))
//
//        if (s.size() == 0) {
//            return true;
//        }
...
//        }
//
//        vector<vector<bool>> result(s.size(), vector<bool>(t.size(), false));
//
//        for (int i = 0; i < s.size(); i++) {
...
//                }
//                else if (i == 0) {
//                    result[i][j] = result[i][j - 1] || s[i] == t[j];
//                }
//                else if (j == 0) {
//                    result[i][j] = false;
//                }
//                else {
//                    result[i][j] = result[i][j - 1] || (s[i] == t[j] && result[i - 1][j - 1]);
//                }
//            }
//        }
//
//        return result[s.size() - 1][t.size() - 1];
//    }
//
//    bool isSubsequence3(string s, string t) {
//        //f(i,j)//s中i 是否是 t中j的子序 = f(i, j - 1) || (s[i] == t[j] && f(i - 1, j - 1))
//
//        vector<vector<bool>> result(s.size() + 1, vector<bool>(t.size() + 1, false));
//        result[0][0] = true;
//        for (int i = 1; i <= s.size(); i++) {
//            for (int j = 1; j <= t.size(); j++) {
//                result[i][j] = result[i][j - 1] || (s[i - 1] == t[j - 1] && result[i - 1][j - 1]);
//            }
//        }
//
//        return result[s.size() - 1][t.size() - 1];
...
//        //f(i,j)//s中i在t中j 子序列长度 f(i,j) = if s[i] == t[j] f(i - 1, j - 1) + 1 else f(i, j - 1)
//
...
//        }
//
//        return result[s.size()][t.size()] == s.size();
//    }
//
//    void test() {
//        /**
...
//
//         //        tool.printMatrix(result);
//                 vector<vector<bool>> :: iterator it = result.begin();
//
//                 while (it != result.end()) {
...
//                 }
//         */
//        bool result = isSubsequence3("abcz", "adsbweqrqwc");
//
//        cout<<result<<endl;
//    }
//};
...
//        //找公共子序列长度
//        //f(i,j)//1中前i个 和 word2中前j个 公共子序列长度
//        //f(i,j) = if word1[i] == word2[j] f(i - 1, j - 1) + 1 else max(f(i - 1, j), f(i, j - 1));
//
...
//        }
//        tool.printMatrix(result);
//        int count = result[word1.size()][word2.size()];
//
//        return (int)word1.size() + (int)word2.size() - 2 * count;
//    }
//
...
//        //优化
//
//        vector<int> result(word2.size() + 1, 0);
//
//
//        int maxLength = 0;
//        for (int i = 1; i <= word1.size(); i++) {
...
//        }
//
//        return (int)word1.size() + (int)word2.size() - 2 * maxLength;
//    }
//
//    int minDistance3(string word1, string word2) {
//        //f(i,j)//word1中i 和 word2中j 最小距离
//        //f(i, j) = if word1[i] == word2[j] : f(i - 1, j - 1) else min(f(i - 1, j) + 1, f(i, j - 1) + 1, f(i - 1, j - 1) + 2)
//
//        vector<vector<int>> result(word1.size() + 1, vector<int>(word2.size() + 1, 0));
//
...
//                }
//                else {
//                    result[i][j] = min(result[i - 1][j] + 1, result[i][j - 1] + 1);
//                    result[i][j] = min(result[i][j], result[i - 1][j - 1] + 2);
//                }
//            }
//        }
//
...
//         "etco"
//         */
////        int result1 = minDistance("leetcode", "etco");
////        int result2 = minDistance2("leetcode", "etco");
//        int result = minDistance3("seaaaaaaaaaa", "eat");
//
//        cout<<result<<endl;
//    }
//};
...
//        //f(i,j) s[0-i]中t[0-j]的子序数量
//        //f(i,j) = if s[i] = t[j] : f(i - 1, j - 1) + f(i - 1)(j)//bagg和bag，在i之前可能已经匹配成功，else f(i - 1 , j);//0-j必须完全匹配，所以可以丢弃i，不可以丢弃j
//
//        if (s.size() < t.size()) {
//            return 0;
//        }
...
//        }
//
//        vector<double> result(t.size() + 1, 0);
//
//        result[0] = 1;
//
//        for (int i = 1; i <= s.size(); i++) {
...
//         */
//        int result2 = numDistinct("babg", "bag");
//        int result = numDistinct2("babg", "bag");
//
//        cout<<result<<endl;
//    }
//};
...
//public:
//    int minDistance(string word1, string word2) {
//        //f(i,j) word1 0-i 到word2 0-j 需要的最少操作步骤
//        //word1[i] == word2[j] f(i,j) = f(i - 1)(j - 1);
//        //word1[i] != word2[j] f(i,j) = min{f(i - 1)(j) + 1 ,f(i)(j - 1) + 1 ,f(i - 1)(j - 1) + 1};
//
//        vector<vector<int>> result(word1.size() + 1, vector<int>(word2.size() + 1, 0));
//
...
//                }
//                else {
//                    result[j] = min({result[j], result[j - 1], last}) + 1;
//                }
//                last = temp;
//            }
//            tool.printVector(result);
//        }
//
//        return result[word2.size()];
//    }
//
//
//    void test() {
//
//        int result2 = minDistance("horse", "ros");
//        int result = minDistance2("horse", "ros");
//
//        cout<<result<<endl;
//    }
//};
...
//    }
//
//    int countSubstringsWithCenter(string& s, int l, int r) {
//        int count = 0;
...
//        //f(i,j) i-j是否是回文
//        //f(i,j) = if s[i] == s[j] : f(i + 1, j - 1) else false
//
//        int count = 0;
//
...
//
//                result[i][j] = (s[i] == s[j] && (j - i <= 1 ||result[i + 1][j - 1] ));
////                if (i == j) {
////                    result[i][j] = true;
////                }
////                else if (s[i] == s[j]) {
...
//
//    void test() {
//        int result = countSubstrings2("aaa");
//
//        cout<<result<<endl;
//    }
//};
...
//        //f(i, j) s[i] != s[j] max(f(i + 1, j)//删除i, f(i, j - 1)//删除j)
//
...
////        tool.printMatrix(result);
//
//        return result[(int)s.size() - 1];
//    }
//
//    void test() {
//        int result = longestPalindromeSubseq2("bbbab");
//
//        cout<<result<<endl;
//    }
//};
